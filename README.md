...
```bash
# Construir a imagem Go 1.23.4 (arquitetura atual)
cd 1.23.4
docker build -t go-builder:1.23.4 .

# Construir para múltiplas arquiteturas (requer Docker buildx)
docker buildx build --platform linux/amd64,linux/arm64 -t go-builder:1.23.4 .
```
...
FROM maiconschmitz/go-builder:1.23.4 AS builder
